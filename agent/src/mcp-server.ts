...
server.registerTool(
  'send_message',
  {
    description: 'Send a message to the user on Telegram. This is the ONLY way to communicate — use it for all responses, progress updates, and results. You can call it multiple times. Your final output text is NOT automatically sent; you must explicitly call this tool.',
    inputSchema: { text: z.string().describe('Message text to send (supports Markdown: **bold**, _italic_, `code`, ```block```, [label](url))') },
  },
...
• Next Friday 5pm             → "0 17 * * 5"   one_shot=true`,
    inputSchema: {
      cron: z.string().describe('5-field cron expression in local time. E.g. "0 9 * * *" (daily 9am), "*/30 * * * *" (every 30 min), "0 8 * * 1" (Mondays 8am)'),
      task: z.string().describe('What the agent should do when the job runs. Be specific — include all context needed, as the agent starts fresh each time.'),
      one_shot: z.boolean().optional().describe('If true, fires once at the next cron tick then deactivates. Use for reminders and one-time tasks. Default: false (recurring).'),
    },
  },
